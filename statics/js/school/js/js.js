...
function frame(b,a,f){
	this.id="f"+b;
	this.elem=document.getElementById(this.id);
	this.activeOverlay=-1;this.imgLoaded=false;
	this.overlays=[f];
	var d=1;
	for(d;d<=f;d++){
...
function play(){
	...
	autoPlay=true;
	scrolltimer=setInterval(goToNextFrame,3000)
}
function pause(){
	if(!autoPlay){
		return
	...
	autoPlay=false;
	if(scrolltimer==undefined||scrolltimer==null){return}
	clearInterval(scrolltimer);
	scrolltimer=null
}
function restart(){pause();play()}
function goToNextFrame(){if(autoPlay){var a=nextFrame();press(a,true)}}
function nextFrame(){
	if(c.selectedIndex==bgimages.length){return 1}
	else{
...
function prevFrame(){
	if(c.selectedIndex>1){return((c.selectedIndex*1)-1)}else{
		return document.getElementById("numFrame").value}}
function loadImg(b){
	if(b!=-1){
		var a=c.s.frames[b-1];
		if(a.imgLoaded){return}
	}
...
function hover(a){
	if(a==-1||a==c.selectedIndex){return}
	loadImg(a);
	c.p.buttons[a-1].elem.className="on"
}
function unhit(a){if(a==-1){return}c.p.buttons[a-1].elem.className="off"}
function out(a){
	if(a==-1||a==c.selectedIndex){return}
...
function active(){if(c.selectedIndex==-1){return}
	pause();
	updateFrame(true)
}
function deactive(){
...
function updateFrame(d){
	...
	if(b==null){return}
	if(b.spots==null){return}
	var a=b.spots.length;if(!d){clearOverlay(b)}for(a;a>0;a--){}
}
function clearOverlay(a){
	if(a.activeOverlay!=-1){
		var b=a.overlays[a.activeOverlay-1];
		hideOverlay(b.id);a.activeOverlay=-1
	}
}
...
function showOverlay(b,d){
	...
	if(c.s.frames[c.selectedIndex-1].activeOverlay==d){return}
...
function hide(a){
	if(a==null||a.style.display=="none"){return}
	a.style.display="none"
}
function show(a){
	if(a==null||a.style.display=="block"){return}
	a.style.display="block"
}
function press(b,a){
	if(c.busy||b==c.selectedIndex){return}
	loadImg(b);
	deactive();
	hit(b);
	unhit(c.selectedIndex);
	slide(b,a);c.selectedIndex=b
}
function slide(b,a){
	c.busy=true;
	...
	(a||b>c.selectedIndex)?scrollRight(b,c.selectedIndex):scrollLeft(b,c.selectedIndex)}function scrollRight(d,a){var b=document.getElementById("stage");b.style.left="0px";reposition(a,0);reposition(d,1);reIndex(d,a);scroll(0,-980)}function scrollLeft(d,a){var b=document.getElementById("stage");b.style.left="-980px";reposition(d,0);reposition(a,1);reIndex(d,a);scroll(-980,0)}function reIndex(e,b){var d=2;var a=c.s.frames.length;for(a;a>0;a--){if(a!=e&&a!=b){reposition(a,d++)}}}
function reposition(b,a){
	c.s.frames[b-1].elem.style.left=980*a+"px"
}

//图片自动滚动效果
//interval=50是贴换的速度，数字越大越快。speed=40是滚动的快慢，数字越小越快;
function scroll(d,a){
